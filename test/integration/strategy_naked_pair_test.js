/**
 * Integration test: Naked Pair Strategy
 */

const { chromium } = require('playwright');
const { NAKED_PAIR_BOARD } = require('./test_boards');
...
async function testNakedPair() {
  console.log('Testing Naked Pair Strategy...\n');
  console.log('Board: Row 0 has two cells with matching candidates');
  ...
  try {
    ...
    await importGame(page, NAKED_PAIR_BOARD);
    ...
    const hasStrategy = await page.evaluate(() => {
      const semantics = document.querySelectorAll('flt-semantics');
      for (const sem of semantics) {
        if (sem.textContent.includes('Naked Pair')) {
          return true;
        }
      }
      return false;
    ...
    if (!hasStrategy) {
      throw new Error('Naked Pair strategy not found in picker');
    }
    console.log('✓ Naked Pair strategy found');

    await clickByText(page, 'Naked Pair');
    await page.waitForTimeout(2000);
    console.log('✓ Naked Pair selected');

    // Naked Pair has 4 phases: Scan, Pattern, Elimination, Target
    for (let i = 0; i < 3; i++) {
