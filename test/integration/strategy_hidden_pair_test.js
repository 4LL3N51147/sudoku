/**
 * Integration test: Hidden Pair Strategy
 */

const { chromium } = require('playwright');
const { HIDDEN_PAIR_BOARD } = require('./test_boards');
...
async function testHiddenPair() {
  console.log('Testing Hidden Pair Strategy...\n');
  console.log('Board: Box 0 has cells with hidden pair pattern');
  ...
  try {
    ...
    await importGame(page, HIDDEN_PAIR_BOARD);
    ...
    const hasStrategy = await page.evaluate(() => {
      const semantics = document.querySelectorAll('flt-semantics');
      for (const sem of semantics) {
        if (sem.textContent.includes('Hidden Pair')) {
          return true;
        }
      }
      return false;
    ...
    if (!hasStrategy) {
      throw new Error('Hidden Pair strategy not found in picker');
    }
    console.log('✓ Hidden Pair strategy found');

    await clickByText(page, 'Hidden Pair');
    await page.waitForTimeout(2000);
    console.log('✓ Hidden Pair selected');

    // Hidden Pair has 4 phases: Scan, Pattern, Elimination, Target
    for (let i = 0; i < 3; i++) {
